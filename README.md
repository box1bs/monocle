...
## Описание:
Проект был создан для быстрой, многопоточной индексации вебстраниц, на английском языке(из-за относительной простоты лингвистики этого языка), с реализацией всего, возможного функционала с помощью build-in инструментов языка golang, для этих целей был реализован скрепинг с распределением задач:
```go
...
case "script", "style", "iframe", "aside", "nav", "footer": // игнорирование текста внутри мусорных тэгов
	garbageTagStack = append(garbageTagStack, tagName)
```
### При обработке текста учитывается позиция токена в документе(среди стандартизированных токенов) для учета при ранжировании в поиске:
```go
i := 0
...
```
### Таким образом реализована главная функция поиска:
```go
