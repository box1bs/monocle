...
```json
{
	...
	"model_sever_link" : "server_path", //ссылка на сервер с моделями ембеддингов и ранжирования
...
## Ограниченая:
Основные ограничения это число документов которые будут обработаны, это число упирается в различные ошибки по типу 403 в корнях дерева, или же то что страница уже обработана, что может отрезать огромное число потенциальных документов, что можно частично исправить увеличением **max_links_ip_page**, так же скорость скрепинга упирается в *вежливость* робота, а так же в повторные попытки парсинга html, для избегания 429, это сильно замедляет скрепинг, основная проблема, на мой взгляд это дообработка уже посещенных страниц, а точнее ссылок от них, главная проблема в этом, это то, что для извлечения ссылок пришлось бы заново получать и парсить html, для этого я интегрировал lru кэш, в который попадают страницы, содержащие максимальное число ссылок, но даже так, приходится маневрировать между полной обработкой и избеганием *лишней деятельности*, что лишает нас части данных, но сильно ускоряет процесс, к сожелению, точечный ущерб подобного невозможно подсчитать поэтому, остается только предполагать насколько это может исказить результаты.

## Запуск:
Запуска python сервера:
(из корня репозитория)
```bash
cd ./internal/app/semantic_embeddings/
python -m .venv venv
./.venv/scripts/Activate
pip install -no-cache-dir requirements.txt
python ./internal/app/semantic_embeddings/app.py
```
...
./bin/app.exe -config "*config_path*"
```
